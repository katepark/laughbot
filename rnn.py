...
class CTCModel():
    ...
    def add_prediction_op(self):
        ...
        W = tf.get_variable(name="W", shape=[Config.num_hidden, Config.num_classes], dtype=tf.float32, initializer=tf.contrib.layers.xavier_ini    tializer())
    ...
    def add_loss_op(self):
        ...
        ctc_loss = tf.nn.ctc_loss(self.targets_placeholder, self.logits, self.seq_lens_placeholder, preprocess_collapse_repeated=False, ctc_mer    ge_repeated=False)
        all_vars = tf.trainable_variables()
        train_vars = []

        #Don't include biases in L2 regularization
        for v in train_vars:
                if tf.shape(v) != 1:
                        train_vars.append(v)
        l2_cost += tf.nn.l2_loss(train_vars)
        ### END YOUR CODE
